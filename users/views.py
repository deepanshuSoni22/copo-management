...
def is_admin(user):
    return user.is_authenticated and user.profile.role == UserRole.ADMIN # Use UserRole directly here
...
def user_update(request, pk):
    ...
    if user.is_superuser and request.user != user:
        messages.error(request, "Superuser status can only be managed by another superuser in Django admin directly.")
        return redirect('user_list')
    if request.user == user and request.user.is_superuser: # Prevent superuser from downgrading/deleting self
        messages.warning(request, "You cannot modify your own superuser status or delete yourself via this interface.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"User '{updated_user.username}' updated successfully with role '{updated_user.profile.get_role_display()}'!")
            return redirect('user_list')
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
        form = UserUpdateForm(instance=user, request=request) # Pass request to the form
    context = {
        'form': form,
...
def user_delete(request, pk):
    ...
    if request.user == user:
        messages.error(request, "You cannot delete your own user account via this interface.")
        return redirect('user_list')
    # Prevent deleting superusers via this interface (force to Django Admin if needed)
    if user.is_superuser:
        messages.error(request, "Superusers cannot be deleted via this interface. Please use Django admin.")
        return redirect('user_list')

    if request.method == 'POST':
        user.delete()
        messages.success(request, f"User '{user.username}' and their profile deleted successfully.")
        return redirect('user_list')
    context = {
        'target_user': user,
    }
