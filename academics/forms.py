...
from .models import AcademicYear, Department, ProgramOutcome, Course, CourseOutcome, COPOMapping, CorrelationLevel, AssessmentType, Assessment, StudentMark
from users.models import UserProfile, UserRole # Import UserProfile from the users app
from django.forms import inlineformset_factory, modelformset_factory # Import formset factories
...
class AcademicYearForm(forms.ModelForm):
    ...
    start_date = forms.DateField(
        widget=forms.DateInput(attrs={'type': 'date', 'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base'}),
        label='Start Date'
    )
    ...
    is_current = forms.BooleanField(
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'form-checkbox h-5 w-5 text-indigo-600 rounded focus:ring-indigo-500'}),
    ...
    class Meta:
        ...
        widgets = {
            ...
            'name': forms.TextInput(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base'}),
        }
    ...
    def clean(self):
        ...
        if is_current:
            ...
            if existing_current.exists():
                self.add_error('is_current', 'Another academic year is already set as current. Please unmark it first.')
...
class DepartmentForm(forms.ModelForm):
    ...
    hod = forms.ModelChoiceField(
        queryset=UserProfile.objects.filter(role='HOD').select_related('user'), # Only show HOD profiles
...
class ProgramOutcomeForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'code': forms.TextInput(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base'}),
...
class CourseForm(forms.ModelForm):
    ...
    faculty = forms.ModelMultipleChoiceField(
        queryset=UserProfile.objects.filter(role=UserRole.FACULTY).select_related('user'),
        widget=forms.SelectMultiple(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base h-32'}), # Increased height for multiple selection
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['faculty'].label_from_instance = lambda obj: f"{obj.user.first_name} {obj.user.last_name} ({obj.user.username})" if obj.user.first_name else obj.user.username


class CourseOutcomeForm(forms.ModelForm):
    # Customize fields with Tailwind classes
    course = forms.ModelChoiceField(
        queryset=Course.objects.all().order_by('code'), # Order by course code for selection
    ...
    def clean(self):
        ...
        if course and code:
            if CourseOutcome.objects.filter(course=course, code=code).exclude(pk=self.instance.pk).exists():
...
class COPOMappingForm(forms.ModelForm):
    ...
    correlation_level = forms.ChoiceField(
        ...
        widget=forms.Select(attrs={'class': 'mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm'}),
        label='Correlation'
    )
...
class AssessmentForm(forms.ModelForm):
    ...
    assesses_cos = forms.ModelMultipleChoiceField(
        ...
        widget=forms.SelectMultiple(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base h-32'}),
    ...
    class Meta:
        model = Assessment
        fields = ['name', 'course', 'academic_year', 'assessment_type', 'max_marks', 'date', 'assesses_cos']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base'}),
            'max_marks': forms.NumberInput(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base', 'step': '0.01'}),
        }
...
class StudentMarkForm(forms.ModelForm):
    ...
    student = forms.ModelChoiceField(
        queryset=User.objects.all().order_by('username'), # Customize this if you have a Student profile
        widget=forms.HiddenInput(), # Hidden input for student
        required=True
    )
    marks_obtained = forms.DecimalField(
        widget=forms.NumberInput(attrs={'class': 'mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-base', 'step': '0.01'}),
