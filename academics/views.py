...
from .forms import AcademicYearForm, DepartmentForm, ProgramOutcomeForm, CourseForm, CourseOutcomeForm, AssessmentTypeForm, AssessmentForm, StudentMarkForm, StudentMarkFormSet # Form
# Import models and forms
from .models import AcademicYear, Department, ProgramOutcome, Course, CourseOutcome, COPOMapping, CorrelationLevel, AssessmentType, Assessment, StudentMark, CourseOutcomeAttainment, ProgramOutcomeAttainment
...
def is_admin_or_hod(user):
    return user.is_authenticated and (user.is_superuser or user.profile.role in ['ADMIN', 'HOD'])

# --- New User Role Check Helper ---
def is_admin_or_hod_or_faculty(user):
    return user.is_authenticated and (user.is_superuser or user.profile.role in ['ADMIN', 'HOD', 'FACULTY'])
...
def home_view(request):
    ...
    if request.user.is_superuser or request.user.profile.role in ['ADMIN', 'HOD']:
        ...
        latest_po_attainment_year_date = ProgramOutcomeAttainment.objects.aggregate(Max('academic_year__start_date'))['academic_year__start_date__max']
        
        if latest_po_attainment_year_date:
            latest_po_academic_year = AcademicYear.objects.get(start_date=latest_po_attainment_year_date)
            
            latest_po_attainments = ProgramOutcomeAttainment.objects.filter(
                academic_year=latest_po_academic_year
            ).select_related('program_outcome', 'academic_year').order_by('program_outcome__code')
            ...
            for po_att in latest_po_attainments:
                ...
                chart_data.append(float(po_att.attainment_percentage) if po_att.attainment_percentage is not None else 0)
            
            context['po_chart_labels'] = chart_labels
            context['po_chart_data'] = chart_data
            context['po_chart_year'] = f"{latest_po_academic_year.start_date.year}-{latest_po_academic_year.end_date.year}"
            # --- END NEW CHART DATA ---
        else:
    ...
    if request.user.profile.role == 'FACULTY' and not request.user.is_superuser and request.user.profile.role not in ['ADMIN', 'HOD']:
        context['faculty_taught_courses_count'] = request.user.profile.taught_courses.count()
...
def academic_year_create(request):
    ...
    return render(request, 'academics/academic_year_form.html', {'form': form, 'form_title': 'Create Academic Year'})
...
def academic_year_update(request, pk):
    ...
    return render(request, 'academics/academic_year_form.html', {'form': form, 'form_title': 'Update Academic Year'})
...
def academic_year_delete(request, pk):
    ...
    return render(request, 'academics/academic_year_confirm_delete.html', {'academic_year': academic_year})
...
def program_outcome_list(request):
    ...
    return render(request, 'academics/program_outcome_list.html', {'program_outcomes': program_outcomes})
...
def program_outcome_create(request):
    ...
    return render(request, 'academics/program_outcome_form.html', {'form': form, 'form_title': 'Create Program Outcome'})
...
def program_outcome_update(request, pk):
    ...
    return render(request, 'academics/program_outcome_form.html', {'form': form, 'form_title': 'Update Program Outcome'})
...
def program_outcome_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('program_outcome_list')
    return render(request, 'academics/program_outcome_confirm_delete.html', {'program_outcome': program_outcome})
...
def course_list(request):
    ...
    courses = Course.objects.all().select_related('department', 'academic_year').prefetch_related('faculty__user').order_by('code')
...
def course_delete(request, pk):
    ...
    if request.method == 'POST':
        course.delete()
        messages.success(request, f'Course "{course.code} - {course.name}" deleted successfully!')
        return redirect('course_list')
...
def course_outcome_list(request):
    ...
    if is_faculty(request.user) and not is_admin_or_hod(request.user):
        ...
        course_outcomes = CourseOutcome.objects.filter(course__in=taught_courses).select_related('course').order_by('course__code', 'code')
    else: # Admin or HOD sees all
        course_outcomes = CourseOutcome.objects.all().select_related('course').order_by('course__code', 'code')

    return render(request, 'academics/course_outcome_list.html', {'course_outcomes': course_outcomes})
        ...
        if form.is_valid():
            ...
            if is_faculty(request.user) and not is_admin_or_hod(request.user):
                if form.cleaned_data['course'] not in request.user.profile.taught_courses.all():
                    messages.error(request, 'You can only create outcomes for courses you are assigned to.')
                    return render(request, 'academics/course_outcome_form.html', {'form': form, 'form_title': 'Create Course Outcome'})

            co = form.save()
            messages.success(request, f'Course Outcome "{co.code}" for {co.course.code} created successfully!')
...
def course_outcome_update(request, pk):
    ...
    if is_faculty(request.user) and not is_admin_or_hod(request.user):
        if co.course not in request.user.profile.taught_courses.all():
            messages.error(request, 'You do not have permission to update this Course Outcome.')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            co = form.save()
            messages.success(request, f'Course Outcome "{co.code}" for {co.course.code} updated successfully!')
            return redirect('course_outcome_list')
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
    ...
    return render(request, 'academics/course_outcome_form.html', {'form': form, 'form_title': 'Update Course Outcome'})
...
def course_outcome_delete(request, pk):
    ...
    if is_faculty(request.user) and not is_admin_or_hod(request.user):
        if co.course not in request.user.profile.taught_courses.all():
            messages.error(request, 'You do not have permission to delete this Course Outcome.')
            return redirect('course_outcome_list')

    if request.method == 'POST':
        co.delete()
        messages.success(request, f'Course Outcome "{co.code}" for {co.course.code} deleted successfully!')
        return redirect('course_outcome_list')
    return render(request, 'academics/course_outcome_confirm_delete.html', {'course_outcome': co})
...
def copo_mapping_view(request, co_pk):
    ...
    existing_mappings = COPOMapping.objects.filter(course_outcome=course_outcome).values('program_outcome_id', 'correlation_level')
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            if errors:
                messages.error(request, 'There were errors processing some mappings. Please review.')
                # Re-fetch existing mappings to show current state after partial saves/errors
                existing_mappings_dict = {m['program_outcome_id']: m['correlation_level'] for m in COPOMapping.objects.filter(course_outcome=course_outcome).values('program_outcome_id', 'correlation_level')}
...
def assessment_type_list(request):
    ...
    return render(request, 'academics/assessment_type_list.html', {'assessment_types': assessment_types})
...
def assessment_type_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Assessment Type "{form.cleaned_data["name"]}" created successfully!')
            return redirect('assessment_type_list')
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
        form = AssessmentTypeForm()
    return render(request, 'academics/assessment_type_form.html', {'form': form, 'form_title': 'Create Assessment Type'})
...
def assessment_type_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Assessment Type "{form.cleaned_data["name"]}" updated successfully!')
            return redirect('assessment_type_list')
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
        form = AssessmentTypeForm(instance=assessment_type)
    return render(request, 'academics/assessment_type_form.html', {'form': form, 'form_title': 'Update Assessment Type'})
...
def assessment_type_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Assessment Type "{assessment_type.name}" deleted successfully!')
        return redirect('assessment_type_list')
    return render(request, 'academics/assessment_type_confirm_delete.html', {'assessment_type': assessment_type})
...
def assessment_list(request):
    ...
    if is_faculty(request.user) and not is_admin_or_hod(request.user):
        user_profile = request.user.profile
        taught_courses = user_profile.taught_courses.all()
        assessments = Assessment.objects.filter(course__in=taught_courses).select_related('course', 'academic_year', 'assessment_type').prefetch_related('assesses_cos').order_by('-academic_year__start_date', 'course__code', 'date')
    else: # Admin or HOD sees all
        assessments = Assessment.objects.all().select_related('course', 'academic_year', 'assessment_type').prefetch_related('assesses_cos').order_by('-academic_year__start_date', 'course__code', 'date')
        
    return render(request, 'academics/assessment_list.html', {'assessments': assessments})
...
def assessment_update(request, pk):
    ...
    return render(request, 'academics/assessment_form.html', {'form': form, 'form_title': 'Update Assessment'})
...
def assessment_delete(request, pk):
    ...
    if request.method == 'POST':
        assessment.delete()
        messages.success(request, f'Assessment "{assessment.name}" for {assessment.course.code} deleted successfully!')
        return redirect('assessment_list')
    return render(request, 'academics/assessment_confirm_delete.html', {'assessment': assessment})
...
def student_mark_entry(request, assessment_pk):
    ...
    if is_faculty(request.user) and not is_admin_or_hod(request.user):
        if assessment.course not in request.user.profile.taught_courses.all():
             messages.error(request, 'You do not have permission to enter marks for this assessment.')
             return redirect('assessment_list')

    # Get all students (users with the 'STUDENT' role) for marks entry for now.
    enrolled_students = User.objects.filter(profile__role='STUDENT').order_by('username')

    # Initialize the formset (for both GET and POST)
    form_data = []
    for student in enrolled_students:
        existing_mark = StudentMark.objects.filter(assessment=assessment, student=student).first()
        form_data.append({
    ...
    if request.method == 'POST':
        formset = StudentMarkFormSet(request.POST, prefix='marks') # Re-initialize with POST data
        if formset.is_valid():
            ...
            messages.success(request, f'Student marks for {assessment.name} updated successfully!')
            return redirect('assessment_list')
        else:
            # --- DEBUGGING LINES START ---
            print("====================================")
            print("Student Mark Formset is NOT valid!")
            print("Overall Formset Errors:", formset.non_form_errors()) # General formset errors
                ...
                if form.errors: # Check if individual form has errors
                    ...
                    try:
                        ...
                        if 'student' in form.cleaned_data:
                            student_obj_for_error = User.objects.get(pk=form.cleaned_data['student'].pk)
                    ...
                    print(f"Form {i} Errors (for Student: {student_obj_for_error.username if student_obj_for_error else 'N/A'}):", form.errors)
            print("====================================")
            # --- DEBUGGING LINES END ---
...
def calculate_co_attainment_for_course(course_obj, academic_year_obj):
    print(f"\n--- Starting CO Attainment Calculation for Course: {course_obj.code} ({course_obj.name}), Year: {academic_year_obj} ---") # DEBUG
    
    course_outcomes = CourseOutcome.objects.filter(course=course_obj).prefetch_related('assessed_by_assessments__student_marks')
    print(f"  Number of COs for this course: {course_outcomes.count()}") # DEBUG

    if not course_outcomes.exists():
        print("  No Course Outcomes found for this course. Skipping CO attainment.") # DEBUG
        return False # No COs to calculate for this course

    for co in course_outcomes:
        print(f"\n  Processing Course Outcome: {co.code} - {co.description[:30]}...") # DEBUG
        ...
        print(f"    Number of Assessments linked to CO {co.code} (in this course/year): {assessments_for_co.count()}") # DEBUG

        if not assessments_for_co.exists():
            print(f"    No relevant assessments found for CO {co.code}. Attainment will be None.") # DEBUG
            attainment_percentage = None
        else:
            # Aggregate marks from relevant assessments for this CO
            current_co_total_possible_marks = 0 # sum of max_marks * students who took it
            current_co_total_obtained_marks = 0 # sum of actual marks obtained

            for assessment in assessments_for_co:
                print(f"      - Processing Assessment: {assessment.name} (Max Marks: {assessment.max_marks})") # DEBUG
                
                # Get student marks only for this specific assessment
                student_marks_queryset = StudentMark.objects.filter(assessment=assessment)
                num_students_with_marks = student_marks_queryset.count()
                
                print(f"        Students with marks in this assessment: {num_students_with_marks}") # DEBUG

                if num_students_with_marks > 0:
                    sum_obtained_for_assessment = student_marks_queryset.aggregate(total_marks=Sum('marks_obtained'))['total_marks']
                    
                    if sum_obtained_for_assessment is not None:
                        current_co_total_obtained_marks += sum_obtained_for_assessment
                        current_co_total_possible_marks += assessment.max_marks * num_students_with_marks
                        print(f"        Obtained: {sum_obtained_for_assessment}, Possible: {assessment.max_marks * num_students_with_marks}") # DEBUG
                    else:
                        print("        No obtained marks sum (might be all None or 0 in DB, but count > 0).") # DEBUG
                else:
                    print("        No students with marks found for this assessment.") # DEBUG
            ...
            print(f"    Total for CO {co.code}: Obtained={total_obtained_marks_from_assessments}, Possible={total_possible_marks_from_assessments}") # DEBUG

            if total_possible_marks_from_assessments > 0:
                attainment_percentage = (total_obtained_marks_from_assessments / total_possible_marks_from_assessments) * 100
                attainment_percentage = round(attainment_percentage, 2)
                print(f"    Calculated Attainment % for CO {co.code}: {attainment_percentage}") # DEBUG
            else:
                attainment_percentage = None
                print(f"    Total possible marks for CO {co.code} is 0. Attainment set to None.") # DEBUG
        ...
        print(f"  CO Attainment record for {co.code} saved: {co_attainment.attainment_percentage}% (Created: {created})") # DEBUG
    
    print(f"\n--- Finished CO Attainment Calculation for Course: {course_obj.code} ---") # DEBUG
    return True

def calculate_po_attainment_for_academic_year(academic_year_obj):
    print(f"\n--- Starting PO Attainment Calculation for Year: {academic_year_obj} ---") # DEBUG
    ...
    for po in program_outcomes:
        ...
        relevant_mappings = COPOMapping.objects.filter(
            program_outcome=po,
            course_outcome__course__academic_year=academic_year_obj
        ).select_related('course_outcome')
        
        print(f"    Number of relevant CO-PO mappings for PO {po.code}: {relevant_mappings.count()}") # DEBUG

        if not relevant_mappings.exists():
            po_attainment_percentage = None
            print(f"    No relevant mappings found for PO {po.code}. Attainment set to None.") # DEBUG
        else:
            for mapping in relevant_mappings:
                print(f"      - Processing mapping: {mapping.course_outcome.code} -> {mapping.program_outcome.code}") # DEBUG
                co_attainment_obj = CourseOutcomeAttainment.objects.filter(
                    course_outcome=mapping.course_outcome,
                    academic_year=academic_year_obj
                ).first()
                
                if co_attainment_obj and co_attainment_obj.attainment_percentage is not None:
                    print(f"        Found CO Attainment for {mapping.course_outcome.code}: {co_attainment_obj.attainment_percentage}% (Weight: {mapping.correlation_level})") # DEBUG
                    weight = mapping.correlation_level
                    total_weighted_attainment += co_attainment_obj.attainment_percentage * weight
                    total_weight += weight
                else:
                    print(f"        No valid CO Attainment found for {mapping.course_outcome.code}.") # DEBUG
            
            print(f"    Final Totals for PO {po.code}: Weighted={total_weighted_attainment}, Total Weight={total_weight}") # DEBUG

            if total_weight > 0:
                po_attainment_percentage = total_weighted_attainment / total_weight
                po_attainment_percentage = round(po_attainment_percentage, 2)
                print(f"    Calculated Attainment % for PO {po.code}: {po_attainment_percentage}") # DEBUG
            else:
                po_attainment_percentage = None
                print(f"    Total weight for PO {po.code} is 0. Attainment set to None.") # DEBUG
        ...
        print(f"  PO Attainment record for {po.code} saved: {po_attainment.attainment_percentage}% (Created: {created})") # DEBUG

    print(f"\n--- Finished PO Attainment Calculation for Year: {academic_year_obj} ---") # DEBUG
    return True

# --- Views to Trigger Calculation ---

@login_required
@user_passes_test(is_admin_or_hod, login_url='/accounts/login/') # Only Admin/HOD can trigger
def calculate_attainment_view(request):
    ...
    if request.method == 'POST':
        ...
        po_academic_year_id = request.POST.get('po_academic_year') # NEW: Changed to 'po_academic_year'

        # Initialize academic year objects based on which button was clicked
        academic_year_obj_for_co = None
        if co_academic_year_id:
            academic_year_obj_for_co = get_object_or_404(AcademicYear, pk=co_academic_year_id)

        academic_year_obj_for_po = None
        if po_academic_year_id:
            academic_year_obj_for_po = get_object_or_404(AcademicYear, pk=po_academic_year_id)
        ...
        if calc_type == 'co_by_course': # No need for 'and course_id' here, check below
            ...
            if not academic_year_obj_for_co or not co_course_id: # Check for the course ID here too
                messages.error(request, "Please select both an Academic Year and a Course to calculate CO attainment.")
            else:
                course_obj = get_object_or_404(Course, pk=co_course_id)
                with transaction.atomic():
                    success = calculate_co_attainment_for_course(course_obj, academic_year_obj_for_co)
                if success:
                    messages.success(request, f"CO Attainment calculated for Course {course_obj.code} in {academic_year_obj_for_co}!")
                else:
                    messages.error(request, "Failed to calculate CO Attainment. Check logs.")
        
        elif calc_type == 'po_by_year':
            # Check if Academic Year is selected for PO calculation
            if not academic_year_obj_for_po:
                messages.error(request, "Please select an Academic Year to calculate PO attainment by Year.")
            else:
                with transaction.atomic():
                    success = calculate_po_attainment_for_academic_year(academic_year_obj_for_po)
                if success:
                    messages.success(request, f"PO Attainment calculated for Academic Year {academic_year_obj_for_po}!")
                else:
                    messages.error(request, "Failed to calculate PO Attainment. Check logs.")
        else:
...
def po_attainment_report_list(request):
    ...
    # Apply filters from GET parameters
    if selected_academic_year_id:
        po_attainments = po_attainments.filter(academic_year__id=selected_academic_year_id)
